---
title: "Explorations"
description: "Saved Views and Data Models are often starting points for further exploration and analysis. Glean makes it safe and easy to take resources further and share what you've found with the `Explorations` feature."
---

## Starting and resuming an exploration

  ![](/imgs/visualizing_data/explore_exploring.jpg){'{'}: style="max-width:50%"{'}'}
...
![](/imgs/visualizing_data/explore_resume.jpg){'{'}: style="max-width:50%"{'}'}

## Sharing an exploration


  ![](/imgs/visualizing_data/explore_share.jpg){'{'}: style="max-width:50%"{'}'}
...
When someone else accesses your link, they will be brought to the resource with your `Exploration` applied. They will see a color header and the name of the person who shared their `Exploration`.
  ...
  ![screenshot of glean when viewing another user's exploration](/imgs/visualizing_data/explore_view.jpg){'{'}: style="max-width:50%"{'}'}


<Warning> While an `Exploration` link contains configurations such as [Breakouts](/visualizing-data/Breakouts) and [Filters](/visualizing-data/Filter), it does not freeze and preserve the underlying data. If someone accesses an `Exploration` and the underlying data has changed, they might not see what was intended. </Warning>
