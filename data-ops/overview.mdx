...
<Warning> We recommend always first creating a Preview Build to see which resources will be affected prior to running a Deploy.  The build process will automatically delete and create resources in Glean to leave your project in a consistent state.  For example, if you delete a column in a model configuration file, all saved views that refer to that column will be automatically deleted (as happens if you delete a column through the user interface). To use DataOps, save the [configuration](/data-ops/Managing-Resources-With-Code) of one or more of your resources to your local environment or a Git repository. Then, you can create a Build using the [Glean command-line interface (CLI)](/data-ops/Using-the-Glean-CLI), or through the Glean web application by connecting your account to a [Git repository](/data-ops/Git-Integration). </Warning>
