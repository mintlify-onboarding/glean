...
description: "Glean supports using an Identity Provider (IdP) such as Okta to control authentication through single sign-on.
...
<Note> **"Contact us"**-  To initiate setup, contact us at [support@glean.io](mailto:support@glean.io). We'll provide specific instructions and a test environment for setting up Glean with the IdP of your choice. </Note>
...
<Warning> **Email constraints**-  It is required that the primary email of each user assigned to Glean in your IdP matches their Glean user’s email. If this isn’t the case, they won’t be able to log in to their Glean account. </Warning>
...
<Info> **Additional notes on `roles`**-  1. It must be non-empty. SCIM-enabled projects do not have a concept of default roles, as all role assignments must be explicitly managed through your IdP. 2. We treat these role names as case-insensitive, so feel free to capitalize as you wish. </Info>
